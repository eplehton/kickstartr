...
It would be quite to use logical operators to create a new set variable.
...
However, there is a good package for recoding which is has maybe a bit more readable syntax. 
...
```
Notice that age_grp2 is already a factor. Quite often, R converts string to factors by default. 
...
satest$set # the level 0 is still remembered, drop it as unnecessary
...
Create a boxplot. We add some colours this time. 
```{r}
